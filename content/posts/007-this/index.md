...
⚠️ `numbers.sum()` is a method invocation on an object (see [3.](#3-method-invocation)) thus `this` equals `numbers`. `calculate()` function is defined inside `sum()`, so you might expect to have `this` as `numbers` object in when invoking `calculate()` too.  
